...
export function createAccount(accountId) {
    ...
    const transfer = el('div.transfer');
    ...
    function getData(data) {
        ...
        displayLastTenTransactions(transactions, accountId, historyTable, 10)
    ...
    function updateAccountBalance(accountId, token) {
        const cachedData = JSON.parse(localStorage.getItem('arr'));
        if (cachedData !== null && cachedData.payload && cachedData.payload.account === accountId) {
            getData(cachedData);
        } else {
    ...
    setChildren(transfer, [createTransfer(accountId, token, getData), canvasContainer]);
    ...
    canvasContainer.addEventListener('click', () => {
        router.navigate(`/history/${accountId}`);
    })
